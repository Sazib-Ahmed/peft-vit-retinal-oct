...
DATASET_CONFIGS: Dict[str, Dict] = {
    ...
    # Your OCT2017 dataset (often much larger). cache_in_ram=False recommended.
    "OCT2017": {
        "root": r"D:\AIUB\DSP\Code\Datasets\OCT2017\OCT2017",
        "pad_to_square": True,  # set True if your OCT2017 images are not square and need it
        "cache_in_ram": False,  # <-- CRITICAL: stop RAM caching OCT2017
    },
}

# Choose which datasets to run
RUN_DATASETS = ["OCT2017", "C8"]  # (to run OCT2017 first, set ["OCT2017", "C8"])
...
def collect_logits_labels(model: nn.Module, loader: DataLoader) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    with torch.inference_mode():
        for x, y in tqdm(loader, desc="--> Collect logits", leave=False, ascii=True, mininterval=5):
...
def train_one_epoch(model, loader, criterion, optimizer) -> Tuple[float, float, float]:
    ...
    for x, y in tqdm(loader, desc="Train", leave=False, ascii=True, mininterval=5):
        x = x.to(CONFIG["device"], non_blocking=True)
        y = y.to(CONFIG["device"], non_blocking=True)
        optimizer.zero_grad(set_to_none=True)
...
def validate_one_epoch(model, loader, criterion) -> Tuple[float, float, float]:
    ...
    for x, y in tqdm(loader, desc="Val", leave=False, ascii=True, mininterval=5):
        x = x.to(CONFIG["device"], non_blocking=True)
        y = y.to(CONFIG["device"], non_blocking=True)
...
def evaluate_on_loader(model: nn.Module, loader: DataLoader, class_names: List[str]) -> Dict:
    ...
    for x, y in tqdm(loader, desc="Test eval", leave=False, ascii=True, mininterval=5):
        x = x.to(CONFIG["device"], non_blocking=True)
        y = y.to(CONFIG["device"], non_blocking=True)
...
def plot_tsne_stratified(
    ...
    with torch.no_grad():
        for x, y in tqdm(loader, desc="t-SNE stratified feats", leave=False, ascii=True, mininterval=5):
            x = x.to(CONFIG["device"], non_blocking=True)
