...
TQDM_KW = dict(
    ...
    leave=False,
    mininterval=1.0,
    file=sys.stderr,              # tqdm normally uses stderr (good)
    disable=not is_tty(),         # disable when redirected to a file
)
...
DATASET_CONFIGS: Dict[str, Dict] = {
    # Your 8-class C8 dataset (wide-field scans -> pad_to_square=True recommended)
    "C8": {
        "root": r"D:\AIUB\DSP\Code\Datasets\C8\RetinalOCT_Dataset_CLEAN",
        "pad_to_square": True,
        "cache_in_ram": False,   # OK for C8 (smaller). If RAM issues, set False.
    },
    # Your NEH_UT_2021 dataset (often much larger). cache_in_ram=False recommended.
    "NEH_UT_2021": {
        "root": r"D:\AIUB\DSP\Code\Datasets\NEH_UT_2021RetinalOCTDataset\NEH_UT_2021_splits_clean",
        "pad_to_square": True,  # set True if your NEH_UT_2021 images are not square and need it
        "cache_in_ram": False,  # <-- CRITICAL: stop RAM caching NEH_UT_2021
    },
    "OCTDL": {
        "root": r"D:\AIUB\DSP\Code\Datasets\OCTDL\OCTDL_SPLIT_BY_PATIENT",
        "pad_to_square": True,
        "cache_in_ram": False,   # OK for OCTDL (smaller). If RAM issues, set False.
    },
}

# Choose which datasets to run
RUN_DATASETS = ["NEH_UT_2021", "C8"]  # (to run NEH_UT_2021 first, set ["NEH_UT_2021", "C8"])
...
CONFIG = {
    ...
    # IMPORTANT: workers > 0 now supported (spawn-safe fixes below)
    # Start with 2 on Windows; go 4 if stable.
    "num_workers": 4,
...
def set_seeds():
    # Spawn-safe: DO NOT touch torch.cuda.* at import time on Windows.
...
def collect_logits_labels(model: nn.Module, loader: DataLoader) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    with torch.inference_mode():
        for x, y in tqdm(loader, desc="--> Collect logits", **TQDM_KW):
...
def train_one_epoch(model, loader, criterion, optimizer) -> Tuple[float, float, float]:
    model.train()
    total_loss = 0.0
    correct = 0
    total = 0
    ...
    for x, y in tqdm(loader, desc="Train", **TQDM_KW):
        x = x.to(CONFIG["device"], non_blocking=True)
        y = y.to(CONFIG["device"], non_blocking=True)
        optimizer.zero_grad(set_to_none=True)
...
def validate_one_epoch(model, loader, criterion) -> Tuple[float, float, float]:
    ...
    for x, y in tqdm(loader, desc="Val", **TQDM_KW):
        x = x.to(CONFIG["device"], non_blocking=True)
        y = y.to(CONFIG["device"], non_blocking=True)
...
def evaluate_on_loader(model: nn.Module, loader: DataLoader, class_names: List[str]) -> Dict:
    ...
    for x, y in tqdm(loader, desc="Test eval", **TQDM_KW):
        x = x.to(CONFIG["device"], non_blocking=True)
        y = y.to(CONFIG["device"], non_blocking=True)
...
def plot_tsne_stratified(
    ...
    with torch.no_grad():
        for x, y in tqdm(loader, desc="t-SNE stratified feats", **TQDM_KW):
            x = x.to(CONFIG["device"], non_blocking=True)
    ...
    print(f"[INFO] t-SNE using {len(X)} samples "
...
def main():
    ...
    print(f"[CFG] num_workers={CONFIG['num_workers']} (Windows spawn-safe)")
    print("=" * 90)
