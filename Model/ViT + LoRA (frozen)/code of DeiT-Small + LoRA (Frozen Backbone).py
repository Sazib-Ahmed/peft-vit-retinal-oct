"""
DeiT-Small + LoRA (Frozen Backbone) for Retinal OCT Classification
=================================================================
Manual LoRA injection into DeiT attention qkv (NO peft needed)
Backbone frozen; train ONLY LoRA + heads
...
- num_workers default 0 (Windows-safe; avoids RAM duplication)
"""
...
CONFIG = {
    ...
    # --- inference benchmark ---
    "bench_merge_lora": True,          # merge LoRA into qkv for timing
...
# =========================
# 2.0 JSON SERIALIZATION HELPERS (FIX #1)
# =========================
# Why this exists:
# - json.dump cannot serialize torch.device, torch.dtype, tensors, numpy scalars, numpy arrays, etc.
# - Your CONFIG dict contains torch.device -> TypeError during json.dump.
# Fix:
# - Provide a `default=` function to json.dump to convert special objects into strings/lists.
def _json_default(o):
    """
...
class OCTCachedFolderDataset(Dataset):
    ...
    def _load_and_preprocess_uint8(self, path: str) -> torch.Tensor:
        ...
        # FIX: make sure NumPy array is writable (avoids torch warning)
...
class GPUAugmentAndNormalize(nn.Module):
    ...
    Why do this on GPU?
      - If you cache uint8 images in RAM, CPU transforms are slow and memory heavy.
      - Doing transforms on GPU keeps CPU light and speeds up training.
    """
    def __init__(self, mode: str, device: torch.device):
        ...
        if self.use_tv2:
            # OCT-safe-ish augmentations: avoid vertical flip, keep small geometry changes.
            if mode == "train":
                self.pipe = nn.Sequential(
                    ...
                    T2.ToDtype(torch.float32, scale=True),  # uint8 -> float in [0,1]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_tv2:
            # tv2 does dtype conversion + normalization inside the pipeline.
            return self.pipe(x)

        # Kornia expects float in [0,1].
        x = x.float().div(255.0)
        x = self.pipe(x)

        # Normalize (ImageNet stats) after kornia aug.
...
# =========================
# 8. MANUAL LORA FOR QKV
# =========================
class LoRALinear(nn.Module):
    """
    Wraps an nn.Linear with LoRA adapters:

        y = x W^T  +  scaling * (drop(x) A B)
    ...
    Where:
      ...
      - scaling = alpha / r
    """
    def __init__(self, orig: nn.Linear, r: int, alpha: int, dropout: float):
        super().__init__()
        ...
        # Freeze original weights (we only want adapters + heads trainable).
        ...
        # Common LoRA init:
        # - A: Kaiming
        # - B: zeros so that initial delta is ~0 (starts from pretrained baseline)
        nn.init.kaiming_uniform_(self.lora_A, a=math.sqrt(5))
        nn.init.zeros_(self.lora_B)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = self.orig(x)
        lora_out = (self.drop(x) @ self.lora_A) @ self.lora_B
        return out + self.scaling * lora_out

def inject_lora_into_deit_qkv(model: nn.Module, r: int, alpha: int, dropout: float):
    """
    Inject LoRA into each Transformer block's attention qkv layer.

    timm DeiT blocks look like:
      model.blocks[i].attn.qkv  (an nn.Linear producing [q,k,v] concatenated)

    We replace that nn.Linear with LoRALinear(nn.Linear).
    """
...
def forward_logits(model: nn.Module, x: torch.Tensor) -> torch.Tensor:
    """
    Utility: run model and return logits (averaging if DeiT-distilled returns tuple).
    Kept for evaluation/benchmark helper functions.
    """
    out = model(x)
...
def loss_for_distilled(model_out, y, criterion):
    """
    DeiT-distilled training often uses two heads:
      (logits, logits_dist)

    We compute average loss:
      0.5*(CE(logits) + CE(logits_dist))
    """
    if isinstance(model_out, (tuple, list)):
        return 0.5 * (criterion(model_out[0], y) + criterion(model_out[1], y))
    return criterion(model_out, y)

def merge_lora_into_qkv_inplace(model: nn.Module):
    """
    Merge LoRA into qkv weight for inference speed tests:

        W' = W + scaling * (A @ B)^T

    After merging, we replace LoRALinear with the underlying nn.Linear.
    """
    if not hasattr(model, "blocks"):
        return model
    for blk in model.blocks:
        if hasattr(blk, "attn") and hasattr(blk.attn, "qkv"):
            ...
            if isinstance(qkv, LoRALinear):
                delta = (qkv.lora_A @ qkv.lora_B).T * qkv.scaling
                with torch.no_grad():
                    qkv.orig.weight.add_(delta)
                blk.attn.qkv = qkv.orig
    return model

def get_adapter_state_dict(model: nn.Module) -> Dict[str, torch.Tensor]:
    """
    Adapter-only state dict = LoRA params + heads.

    NOTE:
    - We detach and move to CPU for portability.
    - This is optional, just for reporting adapter size.
    """
    sd = {}
    for name, tensor in model.state_dict().items():
        if ("lora_A" in name) or ("lora_B" in name) or name.startswith("head.") or name.startswith("head_dist."):
...
def class_balanced_weights(counts: np.ndarray, beta: float = 0.9999) -> np.ndarray:
    ...
    Then normalize weights to mean ~ 1 for numerical stability.
    """
...
def build_sampler_if_needed(train_labels: List[int], num_classes: int, log_path: str):
    ...
    - Helps when dataset is extremely imbalanced.
    - We enable it if:
        (min class < sampler_min_count) OR (max/min >= sampler_imbalance_ratio)
    """
...
def make_run_paths(dataset_name: str, num_classes: int) -> RunPaths:
    ...
    out_dir = os.path.join(CONFIG["runs_root"], dataset_name, f"deit_small_lora_frozen_{num_classes}c", ts)
    safe_mkdir(out_dir)
    return RunPaths(
        out_dir=out_dir,
...
def create_deit_lora_model(num_classes: int, log_path: str) -> nn.Module:
    log_print(log_path, f"\n[MODEL] Creating {CONFIG['model_name']} + Manual LoRA (frozen={CONFIG['freeze_backbone']})")
    model = timm.create_model(CONFIG["model_name"], pretrained=True, num_classes=num_classes)

    inject_lora_into_deit_qkv(model, r=CONFIG["lora_r"], alpha=CONFIG["lora_alpha"], dropout=CONFIG["lora_dropout"])

    if CONFIG["freeze_backbone"]:
        freeze_backbone_keep_lora_and_heads(model)
    else:
        for n, p in model.named_parameters():
            if n.startswith("head.") or n.startswith("head_dist.") or ("lora_" in n):
                p.requires_grad = True
...
def train_one_epoch(model, loader, criterion, optimizer, aug_train: nn.Module, log_path: str):
    """
    FIX #2 (performance bug):
    - Previously the code did TWO forward passes when DeiT output was a tuple:
        out = model(x)
        logits = forward_logits(model, x)  <-- runs model(x) again (expensive!)
    - Now we compute logits directly from `out` without calling model twice.
    """
    model.train()
    total_loss = 0.0
    correct = 0
    ...
    # GradScaler API differs across torch versions; keep your robust try/except.
    try:
    ...
    for x_u8, y in tqdm(loader, desc="Train", leave=False):
        ...
        # Augment + normalize on GPU.
        # No gradients needed for augmentation ops, so we wrap in no_grad.
        with torch.no_grad():
            x = aug_train(x_u8)
        ...
        with torch.autocast(device_type=CONFIG["device"].type,
            ...
            # ===== FIX #2: derive logits from `out` (no second forward) =====
            if isinstance(out, (tuple, list)):
...
def validate_one_epoch(model, loader, criterion, aug_eval: nn.Module):
    """
    FIX #2 also applied here: do not call model(x) twice.
    """
    model.eval()
    total_loss = 0.0
    correct = 0
...
def train_engine(dataset_name: str, train_loader, val_loader, class_names: List[str], run_paths: RunPaths):
    ...
    model = create_deit_lora_model(num_classes, log_path)

    # Compute class-balanced weights from train labels.
...
def build_dataloaders(dataset_name: str, ds_cfg: Dict, run_paths: RunPaths) -> Tuple[DataLoader, DataLoader, DataLoader, List[str]]:
    ...
    # Build sampler decision (criterion computed again in train_engine for correct device usage).
...
def main():
    print("\n" + "=" * 100)
    print("DeiT-Small + Manual LoRA (Frozen Backbone) OCT Pipeline")
    print("=" * 100)
    ...
    for ds_name in RUN_DATASETS:
        ...
        # =========================
        # FIX #1: safe JSON dump of CONFIG (torch.device etc.)
        # =========================
        ...
        plot_training_curves(history, title=f"{ds_name} DeiT-S Manual-LoRA Frozen", out_dir=run_paths.out_dir)
        ...
        model_for_bench = model
        if CONFIG["bench_merge_lora"]:
            model_for_bench = merge_lora_into_qkv_inplace(model_for_bench)

        x0_u8, _ = next(iter(test_loader))

        bench_lat = benchmark_inference(model_for_bench, x0_u8, batch_size=CONFIG["bench_latency_batch"], aug_eval=aug_eval)
        bench_thr = benchmark_inference(model_for_bench, x0_u8, batch_size=min(CONFIG["bench_throughput_batch"], x0_u8.size(0)), aug_eval=aug_eval)
        ...
        # Summary dump is pure python types, but we still provide default=_json_default for safety.
