"""
DeiT-Base + LoRA (Frozen Backbone) for Retinal OCT Classification
=================================================================
...
# -------------------------
# Helper: pip install (py-safe; works in notebooks too)
# -------------------------
...
CONFIG = {
    ...
    # --- training ---
    # NOTE: DeiT-Base is heavier than DeiT-Small; start lower to avoid OOM.
    "batch_size": 32,         # increase carefully if you have enough VRAM
    "epochs": 100,

    # Paper-style suggestion for LoRA-frozen: smaller LR to avoid oscillation.
    # (You can bump to 5e-4 if stable on your setup.)
    "lr": 1e-4,
    "weight_decay": 1e-4,
    "warmup_epochs": 5,
    "patience": 10,
...
class OCTCachedFolderDataset(Dataset):
    ...
    def _load_and_preprocess_uint8(self, path: str) -> torch.Tensor:
        ...
        # Make sure NumPy array is writable (avoids torch warning)
...
def freeze_backbone_keep_lora_and_heads(model: nn.Module):
    ...
    Freeze all params, then unfreeze:
      ...
      - DeiT classification heads: head and head_dist (if present)
    """
...
def make_run_paths(dataset_name: str, num_classes: int) -> RunPaths:
    ...
    out_dir = os.path.join(CONFIG["runs_root"], dataset_name, f"deit_base_lora_frozen_{num_classes}c", ts)
    safe_mkdir(out_dir)
    return RunPaths(
        out_dir=out_dir,
...
def train_one_epoch(model, loader, criterion, optimizer, aug_train: nn.Module, log_path: str):
    """
    No double forward: use `out` to derive logits (handles tuple outputs).
    """
    model.train()
    total_loss = 0.0
    correct = 0
    ...
    # GradScaler API differs across torch versions; keep robust try/except.
    try:
...
def main():
    print("\n" + "=" * 100)
    print("DeiT-Base + Manual LoRA (Frozen Backbone) OCT Pipeline")
    print("=" * 100)
    print(f"[ENV] torch={torch.__version__} cuda={torch.cuda.is_available()} device={CONFIG['device']}")
    if torchvision is not None and hasattr(torchvision, "__version__"):
    ...
    for ds_name in RUN_DATASETS:
        ...
        # Safe JSON dump of CONFIG (torch.device etc.)
        ...
        plot_training_curves(history, title=f"{ds_name} DeiT-B Manual-LoRA Frozen", out_dir=run_paths.out_dir)
